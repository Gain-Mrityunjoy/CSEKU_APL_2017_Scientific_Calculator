...
class InfixToPostfix{
    ...
    public boolean isOperator(char c){
        char operator[] = { '+', '-', '*', '/', '^', '~', 's', 'c', 't','l','L', '@','#','$', '!','p', '%', ')', '(','a','x','y','z','m','n'};
    ...
    public int priority(char c){
        switch (c) {
            ...
            case '*' : case '/' : return 2;
            case '~' : return 3;
            case '@' : case '!' :case'p': case '^' :case '#':case '$':case 'a': return 4;
            case 's' : case 'c' : case 't' :case 'l':case 'L':case 'x':case 'y':case 'z':case 'm':case 'n': return 5;
        }
        return 0;
    }

    public boolean isOneMath(char c){
        char operator[] = { 's', 'c', 't', '@', '(','l','L','x','y','z','m','n'};
        Arrays.sort(operator);
        if (Arrays.binarySearch(operator, c) > -1)
            return true;
        else return false;
